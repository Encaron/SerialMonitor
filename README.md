...
| ✅ **v2.4.0** | 🟡 传感面板 + 体验修缮 | 8 类传感卡片 + 编辑模式 + 侧栏管理 + 协议筛选 + 主题切换按钮 + 版本静默更新 |
| ✅ **v2.5.0** | 🟡 OLED 绘图 + PC 画板 + 协议预览 | 11 种绘图指令 + F5 增量同步 + 旋转 + 三面板协议预览 + Sensor 详情视图 + 使用示例 draw 组 |
...
| # | 计划 | 难度 |
|:--:|------|:--:|
| ✅ 6 | **OLED 绘图指令**——`[draw,...]` 协议：点/线/矩形/圆/椭圆/三角/弧/文字/清屏 | 中 |
| ✅ 7 | **✏ PC 画板 → STM32 物理屏**——PC 端绘制 + 工具栏 → OLED/LCD 显示 + 导出 C | 中 |
| ✅ 19 | **📡 传感面板**——温度/湿度/气压/状态/电机/电池卡片 + 迷你波形 | 中 |
| ✅ 8 | **📊 调参工作台**——Plot 底部抽屉，拖滑杆同时看波形 | 中 |
| ✅ 10 | **📶 音频频谱页**——PC 端自动 FFT + STM32 CMSIS-DSP `[fft,...]` | 中 |
