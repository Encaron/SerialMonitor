...
namespace 串口助手
{
    ...
    public class KeyPanelViewModel
    {
        ...
        /// <summary>
        /// 创建 WASD 方向控制布局（机器人/小车/云台）
        /// 2 行：W 居中，ASD 第二行，Q/E 旋转第三行
        /// </summary>
        public List<KeyViewModel> CreateWASDLayout()
        {
            var newKeys = new List<KeyViewModel>();
            int gid = NewGroupId();
            double keyW = 58, keyH = 42;

            // Row 0:       W
            newKeys.Add(MakeKey("W", "forward", gid, 1, 0, keyW, keyH));
            // Row 1: A   S   D
            newKeys.Add(MakeKey("A", "left", gid, 0, 1, keyW, keyH));
            newKeys.Add(MakeKey("S", "backward", gid, 1, 1, keyW, keyH));
            newKeys.Add(MakeKey("D", "right", gid, 2, 1, keyW, keyH));
            // Row 2: Q       E
            newKeys.Add(MakeKey("Q", "rotate_left", gid, 0, 2, keyW, keyH));
            newKeys.Add(MakeKey("E", "rotate_right", gid, 2, 2, keyW, keyH));
        ...
        /// <summary>
        /// 创建 F1-F8 功能键布局（模式切换/调试控制）
        /// </summary>
        public List<KeyViewModel> CreateFunctionKeysLayout()
        {
            var newKeys = new List<KeyViewModel>();
            int gid = NewGroupId();
            double keyW = 44, keyH = 34;
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                {
                    Name = label,
                    PressSendMode = "数据包", PressSendValue = label.ToLowerInvariant(),
                    ReleaseSendMode = "无", ReleaseSendValue = "",
                    GroupId = gid, LayoutX = i, LayoutY = 0, Width = keyW, Height = keyH,
                });
            }
        ...
        /// <summary>
        /// 创建逻辑开关对布局（ON/OFF · START/STOP · OPEN/CLOSE）
        /// </summary>
        public List<KeyViewModel> CreateSwitchPairsLayout()
        {
            var newKeys = new List<KeyViewModel>();
            int gid = NewGroupId();
            double keyW = 68, keyH = 38;

            // Row 0: ON    START    OPEN
            newKeys.Add(new KeyViewModel { Name = "ON",    PressSendMode = "数据包", PressSendValue = "on",    ReleaseSendMode = "无", GroupId = gid, LayoutX = 0, LayoutY = 0, Width = keyW, Height = keyH });
            newKeys.Add(new KeyViewModel { Name = "START", PressSendMode = "数据包", PressSendValue = "start", ReleaseSendMode = "无", GroupId = gid, LayoutX = 1, LayoutY = 0, Width = keyW, Height = keyH });
            newKeys.Add(new KeyViewModel { Name = "OPEN",  PressSendMode = "数据包", PressSendValue = "open",  ReleaseSendMode = "无", GroupId = gid, LayoutX = 2, LayoutY = 0, Width = keyW, Height = keyH });
            // Row 1: OFF   STOP    CLOSE
            newKeys.Add(new KeyViewModel { Name = "OFF",   PressSendMode = "数据包", PressSendValue = "off",   ReleaseSendMode = "无", GroupId = gid, LayoutX = 0, LayoutY = 1, Width = keyW, Height = keyH });
            newKeys.Add(new KeyViewModel { Name = "STOP",  PressSendMode = "数据包", PressSendValue = "stop",  ReleaseSendMode = "无", GroupId = gid, LayoutX = 1, LayoutY = 1, Width = keyW, Height = keyH });
            newKeys.Add(new KeyViewModel { Name = "CLOSE", PressSendMode = "数据包", PressSendValue = "close", ReleaseSendMode = "无", GroupId = gid, LayoutX = 2, LayoutY = 1, Width = keyW, Height = keyH });
